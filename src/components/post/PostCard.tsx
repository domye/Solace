...
export function PostCard({
	...
	return (
		<article
			...
			style={
				{ ...style, "--coverWidth": COVER_WIDTH_CSS } as React.CSSProperties
			}
		>
			{/* 移动端布局 */}
			<Link
			...
			<div className="hidden md:flex md:flex-col w-full relative">
				<div
					className={`pl-8 pr-4 pt-5 pb-4 relative ${hasCover ? "w-[calc(100%-var(--coverWidth)-12px)]" : isHalfRow ? "w-full" : "w-[calc(100%-52px-12px)]"}`}
				>
					<Link
						to={articleUrl}
              ...
              before:absolute before:top-[25px] before:left-[16px]"
					>
						...
						<SafeIcon
							...
							size="1.75rem"
							className="text-[var(--primary)] transition-bounce inline absolute translate-y-0.5 opacity-0 group-hover:opacity-100 -translate-x-1 group-hover:translate-x-0"
						/>
					</Link>
					<PostMeta article={article} />
					<div className="transition-smooth text-75 mb-2.5 line-clamp-1 text-sm">
						{summary}
					</div>
					<div className="flex flex-wrap gap-1.5 mt-1.5">
				...
				{hasCover ? (
					...
					>
						...
						<SafeIcon
							icon="material-symbols:chevron-right-rounded"
							size="2.5rem"
							className="absolute z-20 opacity-0 group-hover:opacity-100 scale-50 group-hover:scale-100 text-white transition-bounce inset-0 m-auto"
						/>
						<LazyImage
							src={article.cover_image!}
							alt={article.title}
					...
					!isHalfRow && (
						<Link
							to={articleUrl}
							className="btn-regular w-12 absolute right-2.5 top-2.5 bottom-2.5 hover:bg-[var(--btn-regular-bg-hover)] transition-smooth"
						>
							<SafeIcon
								icon="material-symbols:chevron-right-rounded"
								size="2rem"
